...
class Subdivision extends CActiveRecord
{
	...
	public function tableName()
	{
         ...
         * @return type
         */
        public function getFullName($join = ' -> ', $asArray = false) {
            $subdivisions = array();
            $subdivision = $this;
            while (!is_null($subdivision)) {
                 $subdivisions[] = $subdivision->name;
                 $subdivision = $subdivision->parent;
            }
            krsort($subdivisions); 
            return $asArray ? $subdivisions : join($join, $subdivisions);
        }
	...
	public function relations()
	{
		...
		return array(
			'parent' => array(self::BELONGS_TO, 'Subdivision', 'parent_id'),
			'subdivisions' => array(self::HAS_MANY, 'Subdivision', 'parent_id'),
			'users' => array(self::HAS_MANY, 'User', 'subdivision_id'),
		);
	}
